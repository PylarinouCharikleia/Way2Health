class HealthStatus:
	...
	def checkMessages(message1,message2,message3):
	def warnPatient(patient,warning):
	def updateStatus():

class DietReport:
	def __init__(self):
	def dietToStats(diet):
	def checkStats(stats):
	def sendDietMessage(message):

class Diet:
	def __init__(self,name,vitamins,protein):
		self.name=name
...
class MedicationReport:
	...
	def medicineTosubstances(med):
	def checkSubstances(sub):
	def sendMedicationMessage(mes):
...
class MedicalReport:
	...
	def medicalExamsToStats():
	def checkExamStats(stats):
	def sendMedicalExamsMessage():
