...
def openCalendar(myCal):
    ...
    print("-------------------------------------", myCal.title, "-----------------------------------------------------")
    print("Description:", myCal.description)
    print("Today is:", current, "\n")
...
def viewSelectedDate(myCal, date):
    ...
    print("Enter + to add an event or x to exit or anything to return to current date:")
    add = input()
    if add == "+":
        myCal.addEvent(date)
    elif add == "q":
...
def showEventMenu(myCal):
    if myCal.user.gender == "male" or myCal.user.gender == "Male":
        for c in range(0, len(eventTypes) - 1):
            print(c, eventTypes[c])
    elif myCal.user.gender == "female" or myCal.user.gender == "Female":
