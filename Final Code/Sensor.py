...
class Sensor:
	...
	def difSave(self,count,currentInput):
...
class BloodSensor(BodySensor):
	...
	def printValues(self):
		print ("| 	- BOL (Blood Oxygen Level): {0}%              |".format(self.bol))
		print ("| 	- BG (Blood Glucose): {0} mmol/L            |".format(round(self.bg,2)))
...
class GeneralSensor(BodySensor):
	...
	def printValues(self):
		print ("| 	- BT (Body Temperature): {0} C             |". format(round(self.bt,2)))
		print ("| 	- Skin Perspiration: {0} g/min * m2            |".format(self.skinPerspiration))
...
class RespiratorySensor(BodySensor):
	...
	def printValues(self):
		print("| 	- RP (Respiration Rate): {0} breaths/min      |".format(self.rp))
		print("| 	- Respiration Sounds: {0}                     |".format(self.respiSoundsStat))
...
class CardiacSensor(BodySensor):
	...
	def printValues(self):
		print("| 	- ECG: {0}                                    |".format(self.ecg))
		print("| 	- HR (Heart Rate): {0} beats/min          |".format(round(self.hr,2)))
		print("| 	- BP (Blood Pressure): {0}              | ".format(self.checkBloodPressureStatus()))
		print("| 		 Systolic mm Hg (upper number): {0}|".format(round(self.sysBP,2)))
		print("| 		 Diastolic mm Hg (lower number): {0}|".format(round(self.diaBP,2)))
